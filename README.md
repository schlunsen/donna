...
<img src="./assets/github-banner.png" alt="Donna — AI Pentester with Continuous Scanning" width="100%">

# Donna — Continuous AI Pentesting

**A fork of [Donna by Keygraph](https://github.com/KeygraphHQ/donna) focused on continuous security scanning with a remote-access dashboard.**
...
[Donna](https://github.com/KeygraphHQ/donna) is an autonomous AI pentester that analyzes source code, identifies attack vectors, and executes real exploits against running applications. It's powerful — but it runs as a one-shot CLI tool. You kick off a scan, wait an hour, and read the report.

**This fork turns Donna into a continuous security monitor.** It watches your repositories for changes, automatically runs incremental pentests on what changed, tracks findings across scans, and gives you a web dashboard to monitor everything remotely.
...
### One-shot scan (original Donna behavior)
...
This fork preserves Donna's multi-agent pentest pipeline and adds a continuous orchestration layer on top:
...
| **Pentest Pipeline** | `src/temporal/workflows.ts` | Original Donna pipeline (recon → analysis → exploitation → reporting) |
...
│   │   ├── workflows.ts             # Original pentest pipeline workflow
...
This fork supports all providers from upstream Donna:

| Provider | Setup |
|----------|-------|
| **Anthropic API** (default) | Set `ANTHROPIC_API_KEY` in `.env` |
| **Claude Code OAuth** | Set `CLAUDE_CODE_OAUTH_TOKEN` in `.env` |
| **AWS Bedrock** | Set `CLAUDE_CODE_USE_BEDROCK=1` + AWS credentials ([details](https://github.com/KeygraphHQ/donna#aws-bedrock)) |
| **Google Vertex AI** | Set `CLAUDE_CODE_USE_VERTEX=1` + GCP credentials ([details](https://github.com/KeygraphHQ/donna#google-vertex-ai)) |
| **Router Mode** (experimental) | `ROUTER=true` with OpenAI or OpenRouter keys ([details](https://github.com/KeygraphHQ/donna#experimental---unsupported-router-mode-alternative-providers)) |
...
This tool **actively executes attacks** against target applications. All upstream Donna disclaimers apply:
...
See the [upstream Donna README](https://github.com/KeygraphHQ/donna#%EF%B8%8F-disclaimers) for complete disclaimers.

---

## License

AGPL-3.0 — Same as [upstream Donna](https://github.com/KeygraphHQ/donna).

---

## Acknowledgments

This project is a fork of [Donna by Keygraph](https://github.com/KeygraphHQ/donna). All credit for the core pentest pipeline, agent architecture, and Claude Agent SDK integration goes to the Keygraph team.
